...
> A filled in version of every document in the first list, all describing the
> same invented project.

Filled in examples of the documents described in
[Documents worth having](../01-documents.md). Copy the shape and throw the
content away.
...
In the same order as the first document, which is rough order of how much each
...
carry, which is the convention that document asks for.
